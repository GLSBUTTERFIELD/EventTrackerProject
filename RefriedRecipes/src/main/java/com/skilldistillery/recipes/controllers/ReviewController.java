...
public class ReviewController {
	...
	public Review showReview(@PathVariable("recipeId") int recipeId, @PathVariable("reviewId") int reviewId, HttpServletResponse resp) {
		Review review = null;
		try {
	...
	public Review addNewReview(@PathVariable("recipeId") int recipeId, @RequestBody Review review,
		...
		try {
			...
			if (review == null) {
				...
				resp.setHeader("Location", req.getRequestURL().append("/" + recipeId + "/").append(review.getId()).toString());
			}
		} catch (Exception e) {
			resp.setStatus(HttpServletResponse.SC_BAD_REQUEST); //400
			review = null;
